// Generated from ONNX "/home/edward/Documents/workspaces/dev/model-archive/onnx_models/stardist_2d_dsb2018_fixed.onnx" by burn-import
use burn::prelude::*;
...
impl<B: Backend> Default for Model<B> {
    fn default() -> Self {
        Self::from_file(concat!(env!("CARGO_MANIFEST_DIR"), "/pretrained_models/stardist_2d_dsb2018_fixed.bin"), &Default::default())
    }
}
...
impl<B: Backend> Model<B> {
    ...
    pub fn new(device: &B::Device) -> Self {
        ...
        let maxpool2d2 = MaxPool2dConfig::new([2, 2])
            ...
            .with_dilation([1, 1])
            .init();
        let conv2d5 = Conv2dConfig::new([64, 128], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
        ...
        let maxpool2d3 = MaxPool2dConfig::new([2, 2])
            ...
            .with_dilation([1, 1])
            .init();
        let conv2d7 = Conv2dConfig::new([128, 256], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d8 = Conv2dConfig::new([256, 128], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d9 = Conv2dConfig::new([256, 128], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d10 = Conv2dConfig::new([128, 64], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d11 = Conv2dConfig::new([128, 64], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d12 = Conv2dConfig::new([64, 32], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d13 = Conv2dConfig::new([64, 32], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
        ...
        let conv2d14 = Conv2dConfig::new([32, 32], [3, 3])
            ...
            .with_bias(true)
            .init(device);
        let conv2d15 = Conv2dConfig::new([32, 128], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d16 = Conv2dConfig::new([128, 32], [1, 1])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Valid)
            .with_dilation([1, 1])
            .with_groups(1)
    ...
    pub fn forward(&self, input1: Tensor<B, 1>) -> (Tensor<B, 4>, Tensor<B, 4>) {
        ...
        let transpose7_out1 = conv2d16_out1.permute([0, 2, 3, 1]);
        let conv2d17_out1 = self.conv2d17.forward(relu15_out1);
        let sigmoid1_out1 = burn::tensor::activation::sigmoid(conv2d17_out1);
        let reshape8_out1 = sigmoid1_out1.reshape([1, 512, 512, 1]);
